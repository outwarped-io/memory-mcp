...
def upgrade() -> None:
    ...
    op.execute(f"ALTER TABLE memories ADD CONSTRAINT memories_kind_check CHECK (kind IN {_KINDS_WITH_PLAYBOOK})")
    op.add_column(
        "memories",
    ...
    op.execute(
        "CREATE UNIQUE INDEX ix_memories_macro_per_env ON memories (env_id, lower(macro)) WHERE macro IS NOT NULL"
    )
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE memories DROP CONSTRAINT IF EXISTS memories_kind_check")
    op.execute(f"ALTER TABLE memories ADD CONSTRAINT memories_kind_check CHECK (kind IN {_KINDS_WITHOUT_PLAYBOOK})")
